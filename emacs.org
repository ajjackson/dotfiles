...
First a hack which can hopefully be dropped soon: recently Elpaca
seems to be using the new (Emacs 30) 'sort' which has fewer mandatory
args and uses a smart comparison function which is also new to Emacs 30.
Backport these so Elpaca works. (I raised an [[https://github.com/progfolio/elpaca/issues/550][Issue]] as well.)
#+begin_src elisp
...
;; Setup default argument for sort on older Emacs
(when (< emacs-major-version 30)
  (advice-add 'sort :around
              (lambda (orig list &optional pred)
                (funcall orig list (or pred #'value<)))))
...
Set up a server for emacsclient and export the name to subshells so we
...
#+begin_src elisp :results output none
  ...
  (let* ((system-preferred-themes
          '(("Angel.local" . solarized)
            ...
            ("SCMAC-49K2RN" . birds-of-paradise-plus)))
         (preferred-theme
    ...
    (cond
     ...
     ;; Install birds of paradise if necessary, load,
     ;; then set a darker chocolate brown background
     ;; to match the iTerm theme
     ((equal preferred-theme 'birds-of-paradise-plus)
      (progn
        (use-package birds-of-paradise-plus-theme :ensure t :demand t)
        (elpaca-wait)
        (load-theme 'birds-of-paradise-plus t)
        (set-background-color "#352927")
        )
      )
...
** Zoxide
This zoxide package looks pretty nice but I don't fully understand all the functions...
https://github.com/emacsmirror/zoxide

=zoxide-find-file= seems a worthy contender for the precious =C-c C-f=
binding, while not quite a drop-in for =C-x C-f=. (It creates a
two-step process of finding the directory and then the file; it's fine
really but different workflow/muscle-memory!)
...
This still doesn't seem to load properly. Anyway, we should use
global-text-scale-adjust instead, which was added in Emacs 29 and
includes the modeline and minibuffer.
...
#+BEGIN_SRC elisp
    (use-package avy
      ...
      :config
      (use-package link-hint :ensure t :demand t)
...
*** Snippets
#+begin_src conf :tangle "emacs/.emacs.d/snippets/python-mode/inp" :mkdirp yes
  # -*- mode: snippet -*-
  # name: inp
  # key: <inp
  # --
...
#+begin_src conf :tangle "emacs/.emacs.d/snippets/python-mode/iplt"
  # -*- mode: snippet -*-
  # name: iplt
  # key: <iplt
  # --
...
#+begin_src conf :tangle "emacs/.emacs.d/snippets/python-mode/ipth"
  # -*- mode: snippet -*-
  # name: ipth
  # key: <ipth
  # --
...
maybe?) so pin a stable version from source.
    #+BEGIN_SRC elisp
       (use-package transient
        :ensure (:host github :repo "magit/transient" :tag "v0.13.3")
        :demand t
         )

      (use-package magit
      ;;  :ensure t
       :ensure (:host github :repo "magit/magit" :tag "v4.5.0")
       :demand t
...
#+begin_src elisp
  (if (executable-find "fish")
      (use-package fish-mode :ensure t :commands (fish-mode)))
#+end_src

** Snakemake
#+begin_src elisp
(use-package snakemake-mode :ensure t :commands (snakemake-mode))
#+end_src
...
This doesn't seem to always work. To troubleshoot, check binary was
found with flycheck-verify-checker and if not set the variable
...
*** Ruff
I had a play with Copilot(!) to build a replacement for =python-check=
with ruff, as this _also_ seems to have been killed off. It gained a
...
Activating this and setting up familiar keybinding from python-mode

#+begin_src elisp
(require 'my-ruff) ;; loaded from .emacs.d/lisp

(with-eval-after-load 'python-mode
  (define-key python-mode-map (kbd "C-c C-v") #'my/ruff-check-buffer))
#+end_src
...
#+begin_src elisp :tangle (if-workstation "emacs/.emacs.d/init.el" "no")
  ...
  (use-package org-roam
    :ensure (:host github :repo "org-roam/org-roam" :tag "v2.3.0")
    ;; :ensure t
    :demand t
...
#+begin_src elisp
  ...
  ;; (use-package citar-org :ensure t :demand t)
#+end_src
...
* Cleanup
Things seem to be a bit janky at the moment unless I run sml/setup again
#+begin_src elisp  :tangle "emacs/.emacs.d/init.el" :mkdirp yes
  (sml/setup)
#+end_src
...
Prevent byte-compiling because it breaks elpaca
