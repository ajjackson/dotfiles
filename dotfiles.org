...
#+begin_src emacs-lisp
  ...
  (defmacro if-workstation (arg1 arg2)
    `(if (member ,system '("Arctopus" "scpc041" "ajj-mbp-1" "SCLT452Mac" "SCMAC-49K2RN", "toolbx"))
         ,arg1 ,arg2))
#+end_src
...
An ~emacsclient~ wrapper script handles terminal vs GUI frame fallbacks,
passing server names when running nested inside Emacs, and applying
glibc malloc debug preloads where needed. Tangled to the ~emacs~ stow package
so it is linked to =~/.local/bin/em_wrap= whenever emacs dotfiles are stowed.

#+begin_src sh :tangle "emacs/dot-local/bin/em_wrap" :mkdirp yes :shebang "#!/bin/sh"
  export ALTERNATE_EDITOR=""

  PRELOAD_LIB=""
  for lib in /usr/lib64/libc_malloc_debug.so.0 \
             /usr/lib/libc_malloc_debug.so \
             /usr/lib/*-linux-gnu/libc_malloc_debug.so; do
      if [ -f "$lib" ]; then
          PRELOAD_LIB="$lib"
          break
      fi
  ...
  if [ -n "$PRELOAD_LIB" ]; then
      exec env LD_PRELOAD="$PRELOAD_LIB" emacsclient $FRAME_ARG "$@"
  else
      exec emacsclient $FRAME_ARG "$@"
  fi
#+end_src
...
*** Paths
Some systems don't set =~/.local/bin= or =/usr/bin= automatically.
Ensure these directories are always present on PATH for both interactive
and non-interactive sessions (e.g. tools invoked by Emacs, Git hooks, or scripts).
...
**** Editor
    ...
    LD_PRELOAD is handled by the ~em_wrap~ script when using Emacs from
    conda-forge on a recent Linux version. See
    https://github.com/conda-forge/emacs-feedstock/issues/63

    #+BEGIN_SRC bash
      export EDITOR="em_wrap"
      export ALTERNATE_EDITOR=""
    #+END_SRC
