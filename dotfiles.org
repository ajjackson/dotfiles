...
#+begin_src sh :shebang "#!/bin/sh" :tangle (if dmenu-patched-p "i3/.local/bin/dmenu_run" "no")
dmenu_path | dmenu -c -l 6 -g 3 "$@" | ${SHELL:-"/bin/sh"} &
#+end_src

Launch dmenu or i3-dmenu-desktop with shortcuts

#+begin_src conf :tangle (if dmenu-patched-p "i3/.config/i3/config" "no")
  bindsym $mod+d exec --no-startup-id dmenu_run
  bindsym $mod+Shift+d exec --no-startup-id i3-dmenu-desktop --dmenu='dmenu -i -c -l 6 -g 3'
#+end_src
#+begin_src conf :tangle (if dmenu-patched-p "no" "i3/.config/i3/config")
