...
shells running scripts etc.  It's generally recommended that PATH
stuff happens in bash_profile and aliases/functions happen in
bashrc. This is due to the way they are inherited: we don't want
~export PATH=blah:$PATH~ to be extended in every subshell, and
