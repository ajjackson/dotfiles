...
#+begin_src fish :tangle (if (not (equal system "Arctopus")) "fish/.config/fish/config.fish" "no")
...
#+begin_src fish
  if test -f $HOME/src/mantid/.git/config
      ...
      git config alias.build '!bash -cl "$MAMBA_EXE activate mantid-developer && cd ~/src/mantid && cmake --preset=linux && cd build && ninja"'
      git config alias.build-tests '!bash -cl "$MAMBA_EXE activate mantid-developer && cd ~/src/mantid && cmake --preset=linux && cd build && ninja AllTests"'
      git config alias.ctest '!bash -cl "$MAMBA_EXE activate mantid-developer && cd ~/src/mantid/build && ctest $*"'
      git config alias.systemtest '!bash -cl "$MAMBA_EXE activate mantid-developer && cd ~/src/mantid/build && ./systemtest $*"'
      popd
  end
#+end_src
